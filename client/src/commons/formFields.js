...
export class BootstrapSelectField extends React.Component {
  ...
  render() {
    ...
    return (
      <Field validate={validate} field={field}>
        {fieldApi => {
          const { onChange, onBlur, field, label, validate, data, ...rest } = this.props; // eslint-disable-line
          const { value, error, setValue, setTouched } = fieldApi;
            ...
            <FormGroup row>
              ...
              <Col sm={9}>
                <Input
                  ...
                  invalid={error !== undefined}
                >
                  {data.map(this.renderOptionItem)}
                </Input>
                <FormFeedback>{error}</FormFeedback>
              </Col>
            </FormGroup>
